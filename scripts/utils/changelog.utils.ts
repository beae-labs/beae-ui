...
export function getPrData(pr: PullRequest): PrData | undefined {
  ...
  const match = content.match(/## @beae-ui\/vue\@(?<version>\d.+)/)
  ...
  const body = [
    ...
    `description: Explore the changelog for Beae UI version ${version}. Learn about the latest features, bug fixes, and improvements.`,
...
export async function getLatestPr(): Promise<PullRequest> {
  const { data } = await octokit.pulls.list({
    ...
    repo: "beae-ui",
    base: "main",
    head: "beae-ui:changeset-release/main",
    per_page: 1,
  })
