...
export const usePanEvent = (
  ...
  const panSession = ref<PanEvent | null>(null)

  const handlersRef = ref<Partial<PanEventHandlers>>({
  ...
  watchEffect(() => {
    panSession.value?.updateHandlers(handlersRef.value)
  })

  watchEffect(() => {
    const node = reference.value //TODO: need .$el to access dom

    if (!node || !hasPanEvents) return

    function onPointerDown(event: AnyPointerEvent) {
      panSession.value = new PanEvent(event, handlersRef.value, threshold)
    }
