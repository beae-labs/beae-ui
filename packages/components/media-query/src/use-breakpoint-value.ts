...
import { arrayToObjectNotation, fromEntries, isArray } from "@beae-ui/utils"
import { getClosestValue } from "./media-query.utils"
import { useBreakpoint } from "./use-breakpoint"

/**
 * Vue hook for getting the value for the current breakpoint from the
...
export function useBreakpointValue<T = any>(
  ...
  const obj = isArray(values)
    ? fromEntries<Partial<Record<string, T>>>(
        Object.entries(
          arrayToObjectNotation(values, breakpoints as string[]),
