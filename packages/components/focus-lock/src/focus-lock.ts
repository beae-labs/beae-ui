...
export interface FocusLockProps {
  /**
   * Element to which to send focus when focus trap has been de-activated
   */
  ...
  /**
   * If `true`, the first focusable element within the `contentRef`
   * will be auto-focused once `LFocusLock` mounts
   */
  autoFocus?: boolean
}

export const FocusLock = defineComponent({
  name: "LFocusLock",
  ...
  setup(props, { attrs, slots, emit }) {
    ...
    return () => {
      ...
      if (!firstChild) {
        warn({
          condition: __DEV__,
          message: `[beae-ui:focus-lock]: Focus lock component expects at least and only one child element.`,
        })
        return
      }
