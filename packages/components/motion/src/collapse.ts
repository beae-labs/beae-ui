...
import { TransitionEasings } from "./motion-utils"
import { warn } from "@beae-ui/utils"
import { genId } from "@beae-ui/utils"
...
export const LCollapse = defineComponent({
  name: "LCollapse",
  props: {
    isOpen: {
  ...
  setup(props, { slots, attrs, emit }) {
    ...
    warn({
      condition: Boolean(
        (props.startingHeight as number) > 0 && props.unmountOnExit,
      ),
    ...
    return () => {
      ...
      return withDirectives(
        h(beae.div, { ...attrs, ref: targetRef }, () => children),
