import { isBrowser, warn, __DEV__ } from "@beae-ui/utils"

let __portal_id__: number = 0

export const createPortalTarget = (name: string = "default") => {
  const portalId = `beae__${name}__portal__${__portal_id__}`
  let target = document.getElementById(portalId)
  if (!target) {
    target = document.createElement("div")
    target.id = `beae__${name}__portal__${__portal_id__++}`
    /**
     * @todo
     *
     ...
     * doesn't cause the page to junk
     */
...
export const ensureTarget = (selector: string) => {
  if (!isBrowser) {
    console.warn(
      `[beae-ui:portal]: The CPortal component can only be used in the document`,
    )
  }

  if (!document.querySelector(selector)) {
    console.warn(
      `[beae-ui:portal]: the portal target "${selector}" could not be found in the document. Portal children may not be rendered`,
    )
  }
}
